...
| Warp | Generic descendant-fallback heuristic | Best-effort. Warp's window AXTitle is stuck at whichever tab last emitted OSC 2 (doesn't follow focus), and its focused-text-area AXValue includes the whole visible buffer (so scrollback containing "claude" / paths bleeds into matching). Reliable for tabs in distinct cwds; multi-tab-same-cwd is brittle and may misroute. No native CLI / AppleScript / per-tab AX signal is available to fix this from outside Warp. | None |
